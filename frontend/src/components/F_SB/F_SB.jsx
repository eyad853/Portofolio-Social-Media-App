...
const F_SB = ({socket , user , darkMode}) => {
            ...
            <div className="flex-1 overflow-y-auto hide-scrollbar pb-4">
                ...
                showingUsers?.length>0?(
                    <div className="w-full">
                        {showingUsers.map((suggestedUser, index)=>{
                            return (
                                <div key={index} className={`h-20 sm:h-24 relative cursor-pointer flex ${darkMode?"hover:bg-neutral-900 text-white border-neutral-700":"hover:bg-neutral-100 text-neutral-800 border-neutral-200"} transition-all duration-300 px-2 sm:px-3 items-center justify-between gap-2 sm:gap-3 border-b`}>

                                    <Link to={`/home/profile/${suggestedUser._id}`} className='absolute inset-0'></Link>

                                    {/* User Info */}
                                    <div className="flex items-center gap-2 sm:gap-2.5 flex-1 min-w-0">
                                        <div className="w-10 h-10 sm:w-12 sm:h-12 overflow-hidden rounded-full border border-neutral-300 flex-shrink-0">
                                            {suggestedUser && suggestedUser.avatar ? (
                                                <img src={suggestedUser?.avatar} className='w-full h-full rounded-full object-cover' />
                                            ) : (
                                    ...
                                    <div className="flex flex-col sm:flex-row gap-1 sm:gap-2 flex-shrink-0">
                                        <div 
                                        onClick={() => sendRequest(suggestedUser._id)}
                                        className={`${hasRequested(suggestedUser._id)?"bg-neutral-500" :"bg-blue-500"} flex cursor-pointer z-20 transform hover:scale-105 transition-all duration-300 justify-center items-center h-7 sm:h-8 w-20 sm:w-28 rounded-md text-xs font-bold text-white`}>
                                            {hasRequested(suggestedUser._id) ? "Cancel" : "Add Friend"}
                                        </div>
                                        
                                        {!hasRequested(suggestedUser._id) && (
                                            <div 
                                            onClick={()=>{
                                                setUsers(prev=>prev.filter(u=>u._id!==suggestedUser._id))
                                            }}
